...
def test_run_hmmsearch_returns_none_on_no_hits(monkeypatch, tmp_path):
    ...
    start, end = infer_multi._run_hmmsearch(hmm_file, "A" * 60)
...
def test_annotate_sequence_fallback_without_hmm_bounds(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(infer_multi, "_classify", lambda *args, **kwargs: (0, 0.9))
    monkeypatch.setattr(infer_multi, "_run_hmmsearch", lambda *args, **kwargs: (None, None))

    model = object()
    idx2ko = {0: "KO"}
    thresholds = {"KO": 0.1}

    hits = infer_multi._annotate_sequence(
        sequence,
        model,
        idx2ko,
        thresholds,
